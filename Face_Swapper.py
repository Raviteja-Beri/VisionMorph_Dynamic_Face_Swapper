...
def transform_3d_face(image, landmarks, replacement_face):
    ...
    mask = cv2.resize(mask, (roi.shape[1], roi.shape[0]))  # Resize mask to match roi
    replacement_face_rgb = cv2.resize(replacement_face_rgb, (roi.shape[1], roi.shape[0]))  # Resize replacement to match roi
